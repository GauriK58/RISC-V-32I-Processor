...
A combinational **Forwarding Unit** resolves most RAW hazards by bypassing results from the `EX/MEM` or `MEM/WB` pipeline registers directly into the EX stage, avoiding unnecessary stalls.

- **EX Hazard** — result needed by the very next instruction; forwarded from `EX/MEM`.
- **MEM Hazard** — result needed two instructions later; forwarded from `MEM/WB`.

**Forwarding signal encoding:** `00` = no forwarding (register file), `10` = forward from `EX/MEM`, `01` = forward from `MEM/WB`. EX-hazard forwarding takes priority over MEM-hazard forwarding.

### Hazard Detection Unit

`ld` results aren't ready until the end of MEM — one cycle too late to forward to an immediately dependent instruction's EX stage. The **Hazard Detection Unit** checks for this load-use hazard between the ID/EX and IF/ID stages and, when detected, **stalls** the pipeline for one cycle (freezing the PC and IF/ID register) while inserting a NOP bubble into ID/EX.

### Control Hazards

The design uses **static "always not-taken" branch prediction**: the processor keeps fetching sequentially (`PC + 4`) until the branch is resolved in the EX stage.

- If the branch turns out to be taken, `ex_branch_taken` triggers both `branch_taken` (PC redirect) and `flush` (discarding the two wrong-path instructions already in IF and ID).
- This gives a **branch misprediction penalty of 2 cycles**.
...
.
├── images/
│   ├── sequential.png       # Sequential datapath diagram
│   └── pipelined.png        # Pipelined datapath diagram
├── sequential/               # Sequential processor RTL + testbenches
├── pipelined/                 # Pipelined processor RTL + testbenches
├── instructions.txt          # Program instructions (input to instruction memory)
├── register_file.txt         # Final register dump + cycle count (simulation output)
├── data_mem.txt              # Final data memory dump (simulation output)
└── README.md
```

> Update this structure to match your actual folder layout before publishing.

## Contributions

**Sequential**
- **Dhanika** – Control Unit, ALU Control, Register File
- **Moksha** – Data Memory, Immediate Generation, Multiplexers
- **Gauri** – PC, Instruction Memory, Adders, ALU

Final integration of the sequential processor: Moksha & Gauri.

**Pipelining**
- **Dhanika** – Pipeline stages and pipeline registers
- **Moksha** – Forwarding Unit and Hazard Detection Unit
- **Gauri** – Final integration, debugging, and verification of the complete pipelined system

The report was prepared collaboratively by all three members.
